...
static int UnitTesting=0;//If 1 then Disables all interactions and forces duals between ai players.
...
int launchBattlehip(){
	...
	int shot[2]={}, gameMode; //gamemode 0=ai v ai, 1=human vs ai, 2=human v human
	...
	while(!gameEnd){
		...
		if(gameMode>=1){
			...
			printf("Enter Firing Coordinates Player 1(Letter Number):\n");
			do{
				char r;
				scanf("%c%d", &r,&shot[1]);
				shot[0]=tolower(r)-'a';
		...
		if(gameMode==2){
			clearScreen();
			fflush(stdin);
			printf("Player 2 Press Enter When Ready!\n");
			getchar();
			fflush(stdin);
		}
		if(DEBUG && gameMode!=0){
		...
		if(gameMode>=2){
			...
			printf("Enter Firing Coordinates Player 2 (Letter Number):\n");
			do{
				char r;
				scanf("%c%d", &r,&shot[1]);
				shot[0]=tolower(r)-'a';
